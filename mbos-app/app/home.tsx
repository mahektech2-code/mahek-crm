...
export default function Home() {
  ...
  /**
   * Starting the day: the photograph, then attendance, then the timer.
   *
   ...
   * accurate enough fix — is recorded as missing and the day starts anyway,
   * because a salesman who cannot mark attendance cannot work. The photograph
   * is the exception: it is not an attachment to the mark, it is the evidence
   * the mark is made of. See `src/data/attendance.ts`.
   *
   ...
   *
   * The FIX is asked for alongside the camera rather than before it. It used
   * to be awaited first, so the person watched a spinner and then took a
   * photograph; now the GPS has the whole length of the photograph to settle,
   * which costs nothing and makes the fix better. Outside the radius the day
   * still starts and the override reason is asked for afterwards — asking
   * first and losing the check-in to a dismissed dialog is exactly the failure
   ...
   * night.
   *
   * **It takes a photograph too, and it took none before.** So a day proved
   * that somebody arrived and proved nothing whatever about when they stopped
   * — which is the half that decides the hours, and the half worth being
   * careless about. A location is still taken and still never required, the
   * same rule as check-in; the photograph is the exception at both ends.
   */
  const endDay = async () => {
    if (!userId || starting) return;
    setStarting(true);
   ...
   *
   * **This one passed `selfieMediaId: null`**, so the second and third
   * check-ins of a day went unphotographed — a hole exactly where the record
   * is least verifiable, since nobody watches an afternoon session begin. It
   * is the same mandatory photograph as the morning's, because they are the
   * same act.
   */
        ...
        <View style={{ marginTop: 14 }}>
          <Pressable
            ...
            <View style={{ flex: 1, minWidth: 0 }}>
              ...
              {/* The camera is NAMED. It opens the moment this is pressed and
                  there is no way past it, so a button that did not mention it
                  would be a button that surprised somebody four times a day. */}
