 ...
 *
 * One entry, and it earns it: `findDuplicate` matches a typed mobile against
 * every number on the book, and the matching cannot be done in SQL — the
 * numbers are stored as they arrived (`+91 98220 11002`, `09822011003`) and
 * `normaliseMobile` is what makes them comparable. It selects three narrow
 * columns and renders NONE of them; the rows go to a matcher and a single
 * answer comes back. A cap here would not be a slower screen, it would be a
 * duplicate shop created because the match was looked for in the first sixty
 * names alphabetically.
 */
const UNCAPPED_ON_PURPOSE = ['SELECT id, name, phone FROM customers WHERE phone IS NOT NULL'];
...
/**
 * The cap is a shared number, not a literal typed per query.
 *
 * Two screens read the book and a third is waiting to; three different
 * hand-typed limits is how one of them ends up at 600.
 */
test('the book cap is one named constant', () => {
  const src = readFileSync(join(DATA, 'customers.ts'), 'utf8');
  assert.match(src, /export const CUSTOMER_PAGE = \d+;/, 'CUSTOMER_PAGE is where the cap lives');
});
