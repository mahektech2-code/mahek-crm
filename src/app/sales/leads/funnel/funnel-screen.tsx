...
import { Banner, FilterChips, MetricRow, Pill, ScreenHeader } from "../../parts";
    ...
    <div className="p-6">
      ...
      */}
      <div className="grid gap-4 xl:grid-cols-2">
        {funnels.map((f) => (
          <Ladder key={f.salesType ?? "legacy"} funnel={f} />
        ))}
      </div>
        ...
        <div className="mt-3">
          <FilterChips
            ...
            options={PERIODS.map((p) => ({
              ...
              href: `/sales/leads/funnel?period=${p}`,
            }))}
          />
        </div>
...
function Ladder({ funnel }: { funnel: RungFunnel }) {
      ...
      <ol className="px-5 py-4">
        {funnel.rungs.map((rung, i) => (
          <li key={rung.stage} className="mb-2 last:mb-0">
            <Link
              href={`/sales/leads?stage=${rung.stage}`}
        ...
        */
        <footer className="border-t border-line bg-canvas px-5 py-3 text-[13px] text-body">
          ...
          {funnel.offLadder.map((r, i) => (
            <span key={r.stage}>
              {i > 0 ? ", " : ""}
              <Link href={`/sales/leads?stage=${r.stage}`} className="text-[#5223E0]">
