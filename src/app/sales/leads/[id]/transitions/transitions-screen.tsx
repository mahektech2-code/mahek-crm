  ...
  return (
    <div className="p-6">
      <ScreenHeader
        ...
        actions={
          <Link
            href={`/sales/leads/${head.customerId}`}
        ...
        <Empty
          ...
          action={
            onFirstPage ? undefined : (
              <Link
                href={`/sales/leads/${head.customerId}/transitions`}
                className="inline-flex h-9 items-center rounded-[4px] border border-line bg-surface px-3.5 text-sm text-body no-underline hover:bg-canvas hover:no-underline"
              >
                Back to the newest
              </Link>
        ...
        <>
          ...
          <div className="mt-4 flex items-center justify-between gap-3">
            ...
            <div className="flex flex-none gap-2">
              ...
              {page.next ? (
                <Link
                  href={`/sales/leads/${head.customerId}/transitions?after=${encodeURIComponent(
                    `${page.next.at}~${page.next.key}`,
                  )}`}
                  className="inline-flex h-9 items-center rounded-[4px] border border-line bg-surface px-3.5 text-sm text-body no-underline hover:bg-canvas hover:no-underline"
                >
                  Older →
