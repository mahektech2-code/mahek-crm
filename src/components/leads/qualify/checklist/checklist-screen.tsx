...
export function ChecklistScreen({
  ...
  const base = leadHref(workspace, "leads/qualify/checklist");
  ...
  return (
    <>
      ...
      {view === "overridden" ? (
        <OverriddenView workspace={workspace}
          rows={overrides}
          total={overrideTotal}
      ...
      {ticking ? (
        <ChecklistModal workspace={workspace} key={ticking.customerId} row={ticking} onClose={() => setTicking(null)} />
      ) : null}
      {advancing ? (
        <AdvanceModal
  ...
  return (
    <Modal open onClose={onClose} title="Qualification checklist" width={720}>
      ...
      <div className="max-h-[52vh] overflow-y-auto rounded-[6px] border border-line">
        {row.checklist.map((c) => (
          <ConditionRow workspace={workspace}
            key={c.id}
            item={c}
