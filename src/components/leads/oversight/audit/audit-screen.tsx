...
function pageHref(workspace: LeadWorkspace, cursor: AuditCursor | null): string {
  if (!cursor) return leadHref(workspace, "leads/oversight/audit");
  const q = new URLSearchParams({ at: cursor.at, id: cursor.id });
  return leadHref(workspace, `leads/oversight/audit?${q.toString()}`);
}
    ...
    <>
        ...
        <Empty
          ...
          action={
            onFirstPage ? undefined : (
              <Link href={pageHref(workspace, null)} className="text-[13px] font-medium text-[#5223E0]">
                Back to the newest
              </Link>
            )
          }
            ...
            <Row key={r.id} striped={i % 2 === 1}>
              ...
              <Cell truncate={210}>
                {r.subjectCustomerId && r.subject ? (
                  <Link href={leadHref(workspace, `leads/${r.subjectCustomerId}`)} className="text-ink">
                    {r.subject}
                  </Link>
                ) : (
      ...
      <div className="mt-3 flex items-center justify-between gap-3">
        ...
        <div className="flex flex-none gap-2">
          ...
          {nextCursor ? (
            <Link
              href={pageHref(workspace, nextCursor)}
              className="rounded-[4px] border border-line bg-surface px-3 py-1.5 text-[13px] text-body no-underline"
            >
              Older
            </Link>
